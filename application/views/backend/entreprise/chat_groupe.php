...
<body id="page-top">
                ...
                <div class="container-fluid mb-4">
                   ...
                   <div class="col-md-12 card">
                       <div class="row card-body">
                           ...
                           <div class="col-md-8">
                            <?php include("objet_basic_chatgroupe.php") ?>
  ...
  <script>
    $(document).ready(function(){
     ...
     {
      $.ajax({
       url:"<?php echo base_url(); ?>entreprise/pagination_users_on_line/"+page,
       method:"GET",
       dataType:"json",
        beforeSend:function()
        {
     ...
     {
        $.ajax({
         url:"<?php echo base_url(); ?>entreprise/fetch_search_user_online_pagination2",
         method:"POST",
         data:{query:query},
          beforeSend:function()
          {
...
<script type="text/javascript">
  $(document).ready(function() {
     ...
     $(document).on('click', '.editer_message', function(event){  
               ...
               event.preventDefault();
               var idgroupe = $(this).attr("code");  
               $.ajax({  
                    url:"<?php echo base_url(); ?>entreprise/fetch_single_chat_groupe",  
                    method:"POST",  
                    data:{code:idgroupe},  
                    dataType:"json",  
                    success:function(data)  
               ...
               var action = $('#action').val();
                ...
                if(message != '')
                {
                  if (action == 'Edit') {
                        ...
                        $.ajax({  
                             url:"<?php echo base_url() . 'entreprise/modification_chat_message'?>",  
                             method:'POST',  
          ...
          $(document).on('click', '.supprimer_message', function(event){
              ...
              if(confirm("Etes-vous sûr de vouloir le supprimer?"))
              {
                      ...
                      $.ajax({
                        url:"<?php echo base_url(); ?>entreprise/supression_chat_message",
                        method:"POST",
