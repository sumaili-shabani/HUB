                        ...
                        <div class="col-md-12">
                            <div class="row">
                               ...
                               <div class="form-group col-md-12">
                                  <label><i class="fa fa-user"></i> Nom de  la start-up</label>
                                     <select  name="Hommes" id="Hommes" class="form-control selectpicker" data-live-search="true">
                                      <?php 
                                      if ($Hommes->num_rows() > 0) {
                                        ?>
                                        <option value="">Selectionnez la start-up</option>
                                        <?php
                                        foreach ($Hommes->result_array() as $key) {
                                          ?>
                                          <option value="<?php echo($key['ceo']) ?>">
                                            <?php echo($key['first_name'].' '.$key['last_name'].' 🏠 '.$key['nom']) ?></option>
                                          <?php
                                        }
                                      }
                                      else{
   ...
   <script type="text/javascript">
     $(document).ready(function() {
      ...
      function load_country_data(page)
          {
            $.ajax({
             url:"<?php echo base_url(); ?>comptable/pagination_view_paiements/"+page,
             method:"GET",
             dataType:"json",
              beforeSend:function()
              {
         ...
         {
            $.ajax({
             url:"<?php echo base_url(); ?>comptable/fetch_search_view_paiements",
             method:"POST",
             data:{query:query},
              beforeSend:function()
              {
            ...
            event.preventDefault();
              ...
              if(confirm("Etes-vous sûre de vouloir le valider?"))
              {
                  ...
                  if(checkbox.length > 0)
                {
                 ...
                 $.ajax({
                      url:"<?php echo base_url(); ?>comptable/invalider_multiple_fausse_tranaction",
                      method:"POST",
                      data:{
          ...
          */
               ...
               if(idpersonne != ''  && date_paie != '' && montant !='' && motif !='')
                {
                  ...
                  if (action =="Add") {
                     ...
                     if (montant >= 1) {
                       ...
                       $.ajax({  
                             url:"<?php echo base_url() . 'comptable/operation_paiement'?>",  
                             method:'POST',  
          ...
          function detail_user(id_user){
            ...
            if (id_user !='') {
              ...
              $.ajax({  
                    url:"<?php echo base_url(); ?>comptable/fetch_single_personne_user",  
                    method:"POST",  
                    data:{id:id_user},  
                    dataType:"json",  
                    success:function(data)  
