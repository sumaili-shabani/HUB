      ...
      $(document).ready(function() {
            ...
            $('.add_button2').click(function(e){ 
                 ...
                 if(nom != '' && date_debit != '' && date_fin != '' && heure_debit != '' && heure_fin != '')
                  {
                    ...
                    if (action =="Add") {
                        ...
                        $.ajax({  
                             url:"<?php echo base_url() . 'user/operation_conference'?>",  
                             method:'POST',  
                    ...
                    if (action == 'Edit') {
                          ...
                          $.ajax({  
                               url:"<?php echo base_url() . 'user/modification_conference'?>",  
                               method:'POST',  
                               data:new FormData(this),  
                               contentType:false,  
                               processData:false,  
             ...
             $(document).on('click', '.update', function(){  
               ...
               $.ajax({  
                    url:"<?php echo base_url(); ?>user/fetch_single_conference",  
                    method:"POST",  
          ...
          $(document).on('click', '.delete', function(event){
              ...
              if(confirm("Etes-vous sûre de vouloir supprimer la cette conférence?"))
              {
                  ...
                  $.ajax({
                      url:"<?php echo base_url(); ?>user/supression_conference",
                      method:"POST",
                      data:{idconference:idconference},
                      success:function(data)
                      {
          ...
          {
            $.ajax({
              url:"<?php echo base_url(); ?>user/pagination_conference_client/"+page,
              method:"GET",
              dataType:"json",
              beforeSend:function()
              {
          ...
          {
            $.ajax({
             url:"<?php echo base_url(); ?>user/search_conference_client",
             method:"POST",
             data:{query:query},
              beforeSend:function()
              {
